...
function PlannedActivitiesSection() {
  type PA = {
    id: number; title: string; description: string | null
    project_name: string | null; section_name: string | null
    activity_category: string | null; activity_type: string | null
    activity_subtype: string | null; side: string | null; weather: string | null
    start_chainage: string | null; end_chainage: string | null
  ...
  const [open, setOpen]         = useState(false)
  const [allItems, setAllItems] = useState<PA[]>([])
  const [loading, setLoading]   = useState(false)
  const [editId, setEditId]     = useState<number | null>(null)
  const [editVals, setEditVals] = useState<Partial<PA>>({})
  const [newVals, setNewVals]   = useState({ ...empty })
  const [saving, setSaving]     = useState(false)
  const [err, setErr]           = useState('')
  ...
  useEffect(() => { if (open && allItems.length === 0) load() }, [open])

  const visible = showInactive ? allItems : allItems.filter(i => i.is_active)
  const activeCount = allItems.filter(i => i.is_active).length

  async function add() {
    if (!newVals.title.trim()) { setErr('Title is required'); return }
    setSaving(true); setErr('')
  ...
  async function del(id: number) {
    ...
    setSaving(true); setErr('')
    const r = await fetch(`/api/planned?id=${id}`, { method: 'DELETE' })
    const d = await r.json()
    if (!r.ok) { setErr(d.error || 'Delete failed'); setSaving(false); return }
    await load(); setSaving(false)
  }
  ...
  const fieldInp = (key: keyof typeof empty, label: string, flex = '1 1 130px') => (
    <div style={{ flex, minWidth: 90 }}>
      ...
      <input type="text" value={newVals[key]} onChange={e => setNewVals(v => ({ ...v, [key]: e.target.value }))} style={inp} />
    </div>
  )
  ...
  return (
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, overflow: 'hidden' }}>
      ...
      <div onClick={() => setOpen(o => !o)} style={{ padding: '11px 16px', borderBottom: open ? `1px solid ${T.border}` : 'none', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', userSelect: 'none' }}>
        ...
        <span style={{ fontWeight: 700, fontSize: '0.88rem', color: T.text, fontFamily: 'var(--font-display)' }}>Planned Activities</span>
        <span style={{ fontSize: '0.68rem', color: T.sub, fontFamily: 'var(--font-mono)' }}>pre-filled templates workers can pick</span>
        <span style={{ marginLeft: 'auto', fontSize: '0.68rem', color: T.sub, fontFamily: 'var(--font-mono)', marginRight: 8 }}>
          {loading ? '…' : allItems.length > 0 ? `${activeCount} active / ${allItems.length} total` : '—'}
        </span>
        <Chevron open={open} />
      </div>
      ...
      {open && (
        <>
          {/* Controls row */}
          <div style={{ padding: '8px 14px', borderBottom: `1px solid ${T.border}`, background: 'rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
              <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} />
              <span style={{ fontSize: '0.72rem', color: T.sub, fontFamily: 'var(--font-mono)' }}>Show inactive</span>
            </label>
          </div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 580 }}>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={4} style={{ padding: '16px 14px', color: T.sub, fontSize: '0.82rem', textAlign: 'center' }}>Loading…</td></tr>
                ) : visible.length === 0 ? (
                  <tr><td colSpan={4} style={{ padding: '16px 14px', color: T.sub, fontSize: '0.82rem', textAlign: 'center' }}>No planned activities yet — add one below</td></tr>
                ) : visible.map(item => (
                    ...
                    {editId === item.id ? (
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                              <div style={{ flex: '2 1 160px' }}>
                                <div style={{ fontSize: '0.58rem', color: T.sub, fontFamily: 'var(--font-mono)', marginBottom: 3, textTransform: 'uppercase' }}>Title *</div>
                                <input type="text" value={editVals.title ?? ''} onChange={e => setEditVals(v => ({ ...v, title: e.target.value }))} style={inp} />
                              </div>
                              <div style={{ flex: '3 1 200px' }}>
                                <div style={{ fontSize: '0.58rem', color: T.sub, fontFamily: 'var(--font-mono)', marginBottom: 3, textTransform: 'uppercase' }}>Description</div>
                                <input type="text" value={editVals.description ?? ''} onChange={e => setEditVals(v => ({ ...v, description: e.target.value }))} style={inp} />
                              </div>
                            </div>
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                              {(['activity_category','activity_type','project_name','section_name','side','weather','start_chainage','end_chainage'] as const).map(k => (
                                <div key={k} style={{ flex: '1 1 110px', minWidth: 90 }}>
                                  <div style={{ fontSize: '0.58rem', color: T.sub, fontFamily: 'var(--font-mono)', marginBottom: 3, textTransform: 'uppercase' }}>{k.replace(/_/g, ' ')}</div>
                                  <input type="text" value={(editVals as any)[k] ?? ''} onChange={e => setEditVals(v => ({ ...v, [k]: e.target.value }))} style={inp} />
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <>
                        <td style={{ padding: '9px 14px' }}>
                          <div style={{ fontSize: '0.84rem', color: T.text, fontWeight: 600 }}>{item.title}</div>
                          {item.description && <div style={{ fontSize: '0.68rem', color: T.sub, marginTop: 2, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.description}</div>}
                          {(item.start_chainage || item.end_chainage) && (
                            <div style={{ fontSize: '0.65rem', color: T.amber, fontFamily: 'var(--font-mono)', marginTop: 2 }}>
                              {item.start_chainage}{item.end_chainage ? ` → ${item.end_chainage}` : ''}
                            </div>
                          )}
                        </td>
                        <td style={{ padding: '9px 14px', fontSize: '0.78rem', color: T.muted }}>
                          {item.activity_category || '—'}
                          {item.activity_type && <span style={{ color: T.sub }}> › {item.activity_type}</span>}
                          {item.activity_subtype && <span style={{ color: T.sub }}> › {item.activity_subtype}</span>}
                        </td>
                        <td style={{ padding: '9px 14px', fontSize: '0.78rem', color: T.muted }}>
                          {item.project_name || '—'}
                          {item.section_name && <span style={{ color: T.sub }}> / {item.section_name}</span>}
                        </td>
                        <td style={{ padding: '6px 10px' }}>
          ...
          {/* Add form */}
          <div style={{ padding: '12px 14px', borderTop: `1px solid ${T.border}`, background: 'rgba(0,0,0,0.10)', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div style={{ flex: '2 1 160px', minWidth: 120 }}>
                <div style={{ fontSize: '0.6rem', color: T.sub, fontFamily: 'var(--font-mono)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Title *</div>
                <input type="text" value={newVals.title} onChange={e => setNewVals(v => ({ ...v, title: e.target.value }))} placeholder="e.g. Morning Paving Run" style={inp} />
              </div>
              <div style={{ flex: '3 1 200px', minWidth: 140 }}>
                <div style={{ fontSize: '0.6rem', color: T.sub, fontFamily: 'var(--font-mono)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Description</div>
                <input type="text" value={newVals.description} onChange={e => setNewVals(v => ({ ...v, description: e.target.value }))} placeholder="Brief description (optional)" style={inp} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              {fieldInp('activity_category', 'Category')}
              {fieldInp('activity_type', 'Type')}
              {fieldInp('project_name', 'Project')}
              {fieldInp('section_name', 'Section')}
              {fieldInp('side', 'Side', '0 1 70px')}
              {fieldInp('weather', 'Weather', '0 1 80px')}
              {fieldInp('start_chainage', 'Start Ch.', '0 1 90px')}
              {fieldInp('end_chainage', 'End Ch.', '0 1 90px')}
              <button onClick={add} disabled={saving} style={{
                padding: '8px 18px', background: T.text, color: T.bg,
                border: 'none', borderRadius: 8, fontSize: '0.82rem', fontWeight: 700,
                cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1,
  ...
  return (
    <PageShell title="Config" subtitle="Manage dropdown data & user accounts">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <GroupLabel>Planned Activities</GroupLabel>
